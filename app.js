/* =========================================================
   CONFIG FIREBASE
   Sustituye por el mismo objeto de config que usas en el
   resto de tus PWAs del proyecto listify-16b5d (Firestore).
   ========================================================= */
...
const COL = db.collection('facturcontrol_movimientos');
    ...
    while(cursor.getFullYear() < anioActual || (cursor.getFullYear()===anioActual && cursor.getMonth() <= mesActual)){
      ...
      if(!yaExiste){
        ...
        nuevos.push({
          ...
          fecha: nuevaFecha, creadoEn: firebase.firestore.FieldValue.serverTimestamp()
        });
      }
...
$('btnGuardarApunte').onclick = async ()=>{
  ...
  try{
    if(editandoId){
      await COL.doc(editandoId).update(data);
    ...
    await cargarYGenerarRecurrentes();
  } catch(err){
    console.error(err);
    toast('Error al guardar: revisa la config de Firebase');
  }
};
...
function initSelectores(){
  ...
  // Preseleccionar el trimestre del cierre más próximo (el último trimestre completo)
  const t = trimestreDeFecha(hoy());
  const tAnterior = t===1 ? 4 : t-1;
  $('selTrimestre').value = tAnterior;
  if(t===1) anioSel.value = anioActual-1;
}
