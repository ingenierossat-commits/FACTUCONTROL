...
function conTimeout(promesa, ms=10000){
  return Promise.race([
    promesa,
    new Promise((_,reject)=>setTimeout(()=>reject(new Error('Tiempo agotado (10s) escribiendo en Firestore — revisa conexión/reglas')), ms))
  ]);
}
...
$('btnGuardarApunte').onclick = async ()=>{
  ...
  try{
    if(editandoId){
      await conTimeout(COL.doc(editandoId).update(data));
      ...
      const ref = await conTimeout(COL.add(data));
      movimientos.push({id: ref.id, ...data, creadoEn: new Date()});
