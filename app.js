...
function netoDesdeTotal(total, ivaPct, sinIva){
  if(sinIva) return total;
  return total / (1 + (ivaPct||0)/100);
}
function actualizarDesglose(){
  const total = parseFloat($('inpTotal').value);
  const pct = parseFloat($('inpIvaPct').value)||0;
  const sinIva = $('inpSinIva').checked;
  if(isNaN(total)){ $('desgloseTexto').textContent = 'Neto: — · IVA: —'; return; }
  if(sinIva){ $('desgloseTexto').textContent = `Neto: ${fmtEUR(total)} · sin IVA`; return; }
  const neto = netoDesdeTotal(total, pct, false);
  const iva = total - neto;
  $('desgloseTexto').textContent = `Neto: ${fmtEUR(neto)} · IVA (${pct}%): ${fmtEUR(iva)}`;
}
function ivaDe(mov){
      ...
      html += `
        <div class="movimiento" data-id="${m.id}">
          <div class="mov-left">
            ...
            <div class="mov-meta">${m.fecha} · total ${fmtEUR(importe)}${m.sinIva?' (sin IVA)':' · neto '+fmtEUR(m.neto)+' + IVA '+(m.ivaPct||0)+'%'}</div>
          </div>
          <div class="mov-right">
...
$('inpSinIva').addEventListener('change', ()=>{ actualizarVisibilidadIva(); actualizarDesglose(); });
$('inpTotal').addEventListener('input', actualizarDesglose);
$('inpIvaPct').addEventListener('input', actualizarDesglose);
...
function abrirModalEdicion(id){
  ...
  $('inpTotal').value = importeConIva(m).toFixed(2);
...
$('btnGuardarApunte').onclick = async ()=>{
  ...
  if(isNaN(total)){ toast('Importe total no válido'); return; }
  if(!fecha){ toast('Falta la fecha'); return; }

  const sinIva = $('inpSinIva').checked;
  const ivaPct = sinIva ? 0 : (parseFloat($('inpIvaPct').value)||0);
  const neto = netoDesdeTotal(total, ivaPct, sinIva);
  const data = {
...
function calcularResumen(){
  ...
  $('valIvaAPagar').textContent = (liquidacion>=0?'+':'') + fmtEUR(liquidacion) + (liquidacion>=0 ? '' : ' (a compensar)');
  $('valIvaAPagar').className = 'summary-value ' + (liquidacion>=0 ? 'cobro' : 'gasto');
